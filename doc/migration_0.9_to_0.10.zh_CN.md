...
- 动态线程池会先完成必要的 worker 创建，再由提交线程在线程池监视器锁之外
  执行接纳回调。扩容路径只是在普通队列容量之外额外预留一个槽位；所有已接纳
  任务仍发布到同一个全局 `Injector`，并不绑定新创建的 worker。因此 worker
  创建失败时不会调用接纳、执行或取消回调。
- `stop()` 的取消边界现在是 worker 领取任务的时刻。任务可能在并发 stop
  等待期间完成接纳，随后发布到全局队列，并在 run 回调前被取消。`StopReport`
  应视为时间点快照；不要把 `submit_job` 返回 `Ok(())` 当作 run 回调已经开始的证明。
